...
Estuary_area <- st_read("../Base Layers/Site_Region_Areas/SLE.kml")
#
plot(Estuary_area[2])
head(Estuary_area)
#
...
Seagrass <- st_as_sf(crop(as(st_read("../Base Layers/Seagrass/Seagrass_Habitat_in_Florida.shp"), "Spatial"),
                          extent(MicroGrid)))
#Check data
plot(Seagrass[5])
head(Seagrass)
...
Seagrass_alt <- st_as_sf(crop(as(st_read("../Base Layers/Seagrass/Seagrass_Habitat_in_Florida.shp"), "Spatial"),
                              extent(Alt_MicroGrid)))
#Check data
plot(Seagrass_alt[5])
head(Seagrass_alt)
st_crs(Seagrass_alt)
...
#
plot(Depth_alt)
st_crs(Site_Grid_working_alt) == st_crs(Depth_alt) #Confirm matching CRS
#
##Subset of grid cells with depth data
Grid_Depth_geo_alt <- st_intersection(Site_Grid_working_alt, st_as_sf(Depth_alt)) %>%
  dplyr::select(MGID, Depth) %>% group_by(MGID) %>%
  summarize(Depth = mean(Depth, na.rm = T)) %>% 
  st_make_valid()
#
...
Grid_Seagrass_geo <- st_intersection(Site_Grid_working, st_as_sf(Seagrass)) %>%
  dplyr::select(MGID, SEAGRASS) %>%
  rename(Seagrass = SEAGRASS) 
#
...
#
#Add Section designations to working df
Site_Grid_working_5 <- full_join(Site_Grid_working_4, 
                                 Grid_Seagrass, by = "MGID") #Add data by ID
#
#Check duplication - remove duplicates favoring open over closed (AP > CA > CR > RE > PD)
...
#
##Subset of grid cells with Seagrass classifications - may take a while to run
Grid_Seagrass_geo_alt <- st_intersection(Site_Grid_alt_working, st_as_sf(Seagrass_clip_alt)) %>%
  dplyr::select(MGID, SEAGRASS) %>%
  rename(Seagrass = SEAGRASS)  %>% st_make_valid()
#
